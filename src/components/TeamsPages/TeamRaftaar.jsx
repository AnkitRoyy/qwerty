...
function HeroParticles() {
  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < count; i++) {
      const orig_r = Math.sqrt(
        positions[i*3]**2 + positions[i*3+1]**2 + positions[i*3+2]**2
      )
      const drift = 1 + Math.sin(t * 0.4 + phases[i]) * 0.08
      const scale = drift
      pos.array[i*3]   = positions[i*3]   * scale
      pos.array[i*3+1] = positions[i*3+1] * scale + Math.sin(t * 0.3 + phases[i]) * 0.05
      pos.array[i*3+2] = positions[i*3+2] * scale
    }
  ...
  return (
    <div style={{ position: "relative" }}>
        ...
        style={{
          ...
          borderRadius: 4,
          border: `1px solid ${hovered ? "rgba(56,189,248,0.35)" : "rgba(56,189,248,0.1)"}`,
      ...
      >
        <div style={{ fontFamily: "'Space Mono',monospace", fontSize: "2.8rem", fontWeight: 700, color: "#e2e8f0", letterSpacing: "-0.03em", lineHeight: 1 }}>
          {value}
        </div>
        {unit && <div style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.6rem", color: "#38bdf8", letterSpacing: "0.15em", marginTop: 4 }}>{unit}</div>}
        <div style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.52rem", letterSpacing: "0.2em", color: "#334155", textTransform: "uppercase", marginTop: 10 }}>{label}</div>
      </motion.div>
    </div>
  )
}

function StatOrbMesh({ hovered, color }) {
  ...
  useFrame(({ clock }) => {
    ...
    if (r1.current) {
      r1.current.rotation.x = t * 0.5 * boost
      r1.current.material.opacity = (hovered ? 0.7 : 0.4) + Math.sin(t * 1.2) * 0.1
    }
    if (r2.current) {
      r2.current.rotation.z = t * 0.35 * boost
      r2.current.material.opacity = (hovered ? 0.5 : 0.25) + Math.sin(t * 0.9 + 1) * 0.08
    }
    if (core.current) {
      const s = hovered ? 1 + Math.sin(t * 2) * 0.05 : 1
      core.current.scale.set(s, s, s)
      core.current.material.opacity = hovered ? 0.18 : 0.07
    }
  })
  ...
  return (
    <>
      <mesh ref={r1}>
        <torusGeometry args={[0.9, 0.018, 3, 120]} />
        <meshBasicMaterial color={color} transparent opacity={0.4} depthWrite={false} />
      </mesh>
      <mesh ref={r2} rotation={[Math.PI / 2.5, 0, Math.PI / 6]}>
        <torusGeometry args={[1.1, 0.012, 3, 120]} />
      ...
      <mesh>
        ...
        <meshBasicMaterial color={color} wireframe transparent opacity={hovered ? 0.2 : 0.07} depthWrite={false} />
      </mesh>
    </>
  )
}
...
const STATS = [
  { value: "42+",  label: "Top Speed",    unit: "km/h", color: "#38bdf8" },
  { value: "18.4", label: "Vehicle Mass", unit: "kg",   color: "#7dd3fc" },
  { value: "250",  label: "Power Output", unit: "W",    color: "#38bdf8" },
  { value: "0.18", label: "Drag Coeff",   unit: "Cd",   color: "#bae6fd" },
]
    ...
    <div style={{ background: "#020c1b", color: "#e2e8f0", minHeight: "100vh", overflowX: "hidden" }}>
      ...
      >
        ...
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "40vh", pointerEvents: "none", background: "linear-gradient(to top, #020c1b, transparent)" }} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 12 }}>
            {STATS.map((s, i) => (
              <motion.div
                ...
                viewport={{ once: true }} transition={{ delay: i * 0.09, duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
          ...
          <p style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.5rem", color: "#1e3a5f", letterSpacing: "0.15em", marginTop: 12, textAlign: "right" }}>
